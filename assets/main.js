    ...
    $('#tambahBuku').onclick = () => {
        ...
        if (isShown && isEdit) {
            if (confirm('Yakin gak jadi edit data?')) {
                $('.input-data>h4').innerText = 'Tambah Data';
...
function updateUI(books) {
    ...
    for (let book of books) {
        ...
        let title = $new('h5', {text: `${book.title}`});
        ...
        let group = $new('button', {
            ...
            text: `${book.isComplete ? 'Belum selesai' : 'Udah dibaca'}`,
            action: moveGroup
        });
    ...
    !finished.hasChildNodes() ? finished.innerHTML = '<p>Daftar kosong nih</p>' : 0;
    !unfinished.hasChildNodes() ? unfinished.innerHTML = '<p>Daftar kosong nih</p>' : 0;
}
...
function editBook(e) {
    ...
    $('.input-data>h4').innerText = 'Edit Data';
